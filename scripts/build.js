import { CacheMedia } from './lib/cache.js'
import { MakeHtml } from './lib/frontend.js'
...
export async function buildIndexes() {
	...
	const files = await fg('**/*.md', { cwd: srcPostPath() })
	const rawJson = []
	...
	await Promise.all(
		files.map(async (f) => {
			...
			const image = unlocalizedImage ? await cacheMedia.localizeImage(unlocalizedImage) : null
	...
	fs.writeFileSync(
		buildPath('raw.json'),
		JSON.stringify(rawJson.reduce((prev, { path, ...p }) => ({ ...prev, [path]: { path, ...p } }), {})),
	)
	fs.writeFileSync(
		buildPath('idx.json'),
		JSON.stringify(
	...
	const mediaFiles = await fg('**/*.*', { cwd: srcMediaPath() })
